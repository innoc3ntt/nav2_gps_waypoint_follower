// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from gps_interfaces:action/FollowGPSWaypoints.idl
// generated code does not contain a copyright notice
#include "gps_interfaces/action/detail/follow_gps_waypoints__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>


// Include directives for member types
// Member `gps_poses`
#include "geographic_msgs/msg/detail/geo_pose__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_Goal__init(gps_interfaces__action__FollowGPSWaypoints_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // gps_poses
  if (!geographic_msgs__msg__GeoPose__Sequence__init(&msg->gps_poses, 0)) {
    gps_interfaces__action__FollowGPSWaypoints_Goal__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Goal__fini(gps_interfaces__action__FollowGPSWaypoints_Goal * msg)
{
  if (!msg) {
    return;
  }
  // gps_poses
  geographic_msgs__msg__GeoPose__Sequence__fini(&msg->gps_poses);
}

bool
gps_interfaces__action__FollowGPSWaypoints_Goal__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Goal * lhs, const gps_interfaces__action__FollowGPSWaypoints_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // gps_poses
  if (!geographic_msgs__msg__GeoPose__Sequence__are_equal(
      &(lhs->gps_poses), &(rhs->gps_poses)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Goal__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Goal * input,
  gps_interfaces__action__FollowGPSWaypoints_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // gps_poses
  if (!geographic_msgs__msg__GeoPose__Sequence__copy(
      &(input->gps_poses), &(output->gps_poses)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_Goal *
gps_interfaces__action__FollowGPSWaypoints_Goal__create()
{
  gps_interfaces__action__FollowGPSWaypoints_Goal * msg = (gps_interfaces__action__FollowGPSWaypoints_Goal *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Goal));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_Goal));
  bool success = gps_interfaces__action__FollowGPSWaypoints_Goal__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_Goal__destroy(gps_interfaces__action__FollowGPSWaypoints_Goal * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_Goal__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_Goal * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_Goal *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_Goal));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_Goal__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_Goal__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence *
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_Goal);
    gps_interfaces__action__FollowGPSWaypoints_Goal * data =
      (gps_interfaces__action__FollowGPSWaypoints_Goal *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_Goal__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_Goal__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `missed_waypoints`
#include "rosidl_runtime_c/primitives_sequence_functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_Result__init(gps_interfaces__action__FollowGPSWaypoints_Result * msg)
{
  if (!msg) {
    return false;
  }
  // missed_waypoints
  if (!rosidl_runtime_c__int32__Sequence__init(&msg->missed_waypoints, 0)) {
    gps_interfaces__action__FollowGPSWaypoints_Result__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Result__fini(gps_interfaces__action__FollowGPSWaypoints_Result * msg)
{
  if (!msg) {
    return;
  }
  // missed_waypoints
  rosidl_runtime_c__int32__Sequence__fini(&msg->missed_waypoints);
}

bool
gps_interfaces__action__FollowGPSWaypoints_Result__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Result * lhs, const gps_interfaces__action__FollowGPSWaypoints_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // missed_waypoints
  if (!rosidl_runtime_c__int32__Sequence__are_equal(
      &(lhs->missed_waypoints), &(rhs->missed_waypoints)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Result__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Result * input,
  gps_interfaces__action__FollowGPSWaypoints_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // missed_waypoints
  if (!rosidl_runtime_c__int32__Sequence__copy(
      &(input->missed_waypoints), &(output->missed_waypoints)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_Result *
gps_interfaces__action__FollowGPSWaypoints_Result__create()
{
  gps_interfaces__action__FollowGPSWaypoints_Result * msg = (gps_interfaces__action__FollowGPSWaypoints_Result *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Result));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_Result));
  bool success = gps_interfaces__action__FollowGPSWaypoints_Result__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_Result__destroy(gps_interfaces__action__FollowGPSWaypoints_Result * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_Result__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_Result * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_Result *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_Result));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_Result__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_Result__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_Result__Sequence *
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_Result__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Result__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Result__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_Result);
    gps_interfaces__action__FollowGPSWaypoints_Result * data =
      (gps_interfaces__action__FollowGPSWaypoints_Result *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_Result__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_Result__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__init(gps_interfaces__action__FollowGPSWaypoints_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_waypoint
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(gps_interfaces__action__FollowGPSWaypoints_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_waypoint
}

bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Feedback * lhs, const gps_interfaces__action__FollowGPSWaypoints_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_waypoint
  if (lhs->current_waypoint != rhs->current_waypoint) {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Feedback * input,
  gps_interfaces__action__FollowGPSWaypoints_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_waypoint
  output->current_waypoint = input->current_waypoint;
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_Feedback *
gps_interfaces__action__FollowGPSWaypoints_Feedback__create()
{
  gps_interfaces__action__FollowGPSWaypoints_Feedback * msg = (gps_interfaces__action__FollowGPSWaypoints_Feedback *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Feedback));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_Feedback));
  bool success = gps_interfaces__action__FollowGPSWaypoints_Feedback__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_Feedback__destroy(gps_interfaces__action__FollowGPSWaypoints_Feedback * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_Feedback * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_Feedback *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_Feedback));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence *
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_Feedback);
    gps_interfaces__action__FollowGPSWaypoints_Feedback * data =
      (gps_interfaces__action__FollowGPSWaypoints_Feedback *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "gps_interfaces/action/detail/follow_gps_waypoints__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__init(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!gps_interfaces__action__FollowGPSWaypoints_Goal__init(&msg->goal)) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  gps_interfaces__action__FollowGPSWaypoints_Goal__fini(&msg->goal);
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__are_equal(const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * lhs, const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!gps_interfaces__action__FollowGPSWaypoints_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__copy(
  const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * input,
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!gps_interfaces__action__FollowGPSWaypoints_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request *
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__create()
{
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * msg = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request));
  bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__destroy(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence *
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request);
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request * data =
      (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__init(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__are_equal(const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * lhs, const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__copy(
  const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * input,
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response *
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__create()
{
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * msg = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response));
  bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__destroy(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence *
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response);
    gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response * data =
      (gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__init(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__are_equal(const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * lhs, const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__copy(
  const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * input,
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_GetResult_Request *
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__create()
{
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * msg = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Request *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request));
  bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__destroy(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Request *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence *
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Request);
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Request * data =
      (gps_interfaces__action__FollowGPSWaypoints_GetResult_Request *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "gps_interfaces/action/detail/follow_gps_waypoints__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__init(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!gps_interfaces__action__FollowGPSWaypoints_Result__init(&msg->result)) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  gps_interfaces__action__FollowGPSWaypoints_Result__fini(&msg->result);
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__are_equal(const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * lhs, const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!gps_interfaces__action__FollowGPSWaypoints_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__copy(
  const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * input,
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!gps_interfaces__action__FollowGPSWaypoints_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_GetResult_Response *
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__create()
{
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * msg = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Response *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response));
  bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__destroy(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Response *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence *
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_GetResult_Response);
    gps_interfaces__action__FollowGPSWaypoints_GetResult_Response * data =
      (gps_interfaces__action__FollowGPSWaypoints_GetResult_Response *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "gps_interfaces/action/detail/follow_gps_waypoints__functions.h"

bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__init(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__init(&msg->feedback)) {
    gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  gps_interfaces__action__FollowGPSWaypoints_Feedback__fini(&msg->feedback);
}

bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__are_equal(const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * lhs, const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__copy(
  const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * input,
  gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!gps_interfaces__action__FollowGPSWaypoints_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage *
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__create()
{
  gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * msg = (gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage));
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage));
  bool success = gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__init(msg);
  if (!success) {
    free(msg);
    return NULL;
  }
  return msg;
}

void
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__destroy(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * msg)
{
  if (msg) {
    gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(msg);
  }
  free(msg);
}


bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__init(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * data = NULL;
  if (size) {
    data = (gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage *)calloc(size, sizeof(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage));
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(&data[i - 1]);
      }
      free(data);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__fini(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(&array->data[i]);
    }
    free(array->data);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence *
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__create(size_t size)
{
  gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * array = (gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence *)malloc(sizeof(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence));
  if (!array) {
    return NULL;
  }
  bool success = gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    free(array);
    return NULL;
  }
  return array;
}

void
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__destroy(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * array)
{
  if (array) {
    gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__fini(array);
  }
  free(array);
}

bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__are_equal(const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * lhs, const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence__copy(
  const gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * input,
  gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage);
    gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage * data =
      (gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__fini(&data[i]);
        }
        free(data);
        return false;
      }
    }
    output->data = data;
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!gps_interfaces__action__FollowGPSWaypoints_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
